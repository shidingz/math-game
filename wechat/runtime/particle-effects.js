'use strict';
// Shared Canvas trajectories for the game and design preview. No model calls.
const GROUPS=[
...
function stamp(ctx,sprite,x,y,w,h){
 if(sprite.image)ctx.drawImage(sprite.image,sprite.sx,sprite.sy,sprite.sw,sprite.sh,x,y,w,h);
...
function paint(ctx,points,{x,y,tinted,front}){
 for(const p of points){if(p.front!==front)continue;const image=tinted(p.group,p.index,p.hueOffset);if(!image)continue;
  ...
  stamp(ctx,image,-p.size/2,-p.size/2,p.size,p.size);ctx.restore();
 }
}
function flow(ctx,{preset,x,y,width,height,level,time,hue,seed=1,front=false,reducedMotion=false,tinted}){
 ...
 for(let j=0;j<number;j++){
  ...
  if(kind!=='ribbons'&&isFront(headP)===front){
   ...
   if(im){ctx.globalAlpha=1;stamp(ctx,im,headP.x-size/2,headP.y-size/2,size,size);}
  }
  if(kind==='ribbons')for(let k=0;k<28;k++){
   ...
   if(im){ctx.globalAlpha=.95;stamp(ctx,im,p.x-size/2,p.y-size/2,size,size);}
  }
 }
 ctx.restore();
}
module.exports={GROUPS,CATALOG,PRESETS,particles,paint,flow,stamp};
