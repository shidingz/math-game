...
test('每种初始宠物都可免费选一次，其他伙伴包括悟空一律200分',()=>{
  for(const id of ids){const s=core.initialState();assert.equal(core.chooseStarter(s,id,ids),true);assert.equal(s.points,0);assert.deepEqual(Object.keys(s.unlockedPets),[id]);assert.equal(s.activePet,id);
    ...
    for(const other of ids.filter(x=>x!==id)){s.points=199;assert.equal(core.unlockPet(s,other).status,'insufficient');s.points=200;assert.equal(core.unlockPet(s,other).cost,200);assert.equal(s.points,0);assert.equal(core.unlockPet(s,other).status,'already-unlocked');}
  }
});
...
test('移除精卫后旧存档保留其他宠物和积分，精卫进度只留作归档',()=>{
  ...
  const s=core.restore(old);assert.equal(s.activePet,'wukong');assert.equal(s.points,520);assert.equal(s.starterChosen,true);assert.deepEqual(s.pets.ragdoll,{level:8,growth:30,feeds:30});assert.deepEqual(s.pets.wukong,{level:6,growth:60,feeds:20});assert.equal(s.pets.jingwei,undefined);assert.equal(s.unlockedPets.jingwei,undefined);assert.deepEqual(s.retiredPets.jingwei,old.pets.jingwei);assert.deepEqual(core.restore(s),s);
});
