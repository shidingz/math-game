...
const root=path.resolve(__dirname,'../characters/bichon');
function setup(){
  ...
  const sandbox={window:{},HTMLElement:Host,CustomEvent,URL,Image:FakeImage,document:{currentScript:{src:'http://local/bichon/bichon.js'},hidden:false},matchMedia:()=>motion,requestAnimationFrame:()=>1,cancelAnimationFrame(){},customElements:{get:n=>registered.get(n),define:(n,c)=>registered.set(n,c)}};
  sandbox.window=sandbox;
  for(const name of ['bichon-data.js','bichon-effects.js','bichon.js'])vm.runInNewContext(fs.readFileSync(path.join(root,name),'utf8'),sandbox);
  const pet=new (registered.get('bichon-game-pet'))();pet.isConnected=true;pet.connectedCallback();
  return {pet,images,sandbox,motion,ready:async()=>{images[0].onload();await Promise.resolve();}};
}
test('比熊犬动作结束后回到待机，暂停后时钟停止',async()=>{const {pet,ready}=setup();await ready();pet.autoIdle=false;pet.play('feed');pet._tick(1);for(let n=51;n<=451;n+=50)pet._tick(n);assert.equal(pet.action,'feed');pet._tick(501);assert.equal(pet.action,'idle');pet.pause(true);const t=pet._t;pet._tick(2901);assert.equal(pet._t,t);});
