...
test('暹罗猫兑换扣200分，喂养独立，刷新后保留进度且默认仍是孙悟空',()=>{
  const ctx=registry(),cfg=ctx.MathPetCharacters.get('siamese'),s=core.initialState({chooseStarter:false});assert.equal(cfg.unlock.cost,200);assert.equal(s.activePet,'wukong');
  s.points=199;const before=structuredClone(s);assert.equal(core.unlockPet(s,'siamese',cfg).status,'insufficient');assert.deepEqual(s,before);
  s.points=220;assert.equal(core.unlockPet(s,'siamese',cfg).status,'unlocked');assert.equal(s.points,20);assert.equal(core.unlockPet(s,'siamese',cfg).status,'already-unlocked');
