...
const root=path.resolve(__dirname,'../characters/samoyed');
function setup(){
  ...
  const sandbox={window:{},HTMLElement:Host,CustomEvent,URL,Image:FakeImage,document:{currentScript:{src:'http://local/samoyed/samoyed.js'},hidden:false},
  ...
  for(const name of ['samoyed-data.js','samoyed-effects.js','samoyed.js'])vm.runInNewContext(fs.readFileSync(path.join(root,name),'utf8'),sandbox);
  const pet=new (registered.get('samoyed-game-pet'))();pet.isConnected=true;pet.connectedCallback();
