...
const root=path.resolve(__dirname,'../characters/ragdoll');
function setup(){
  ...
  const sandbox={window:{},HTMLElement:Host,CustomEvent,URL,Image:FakeImage,document:{currentScript:{src:'http://local/ragdoll/ragdoll.js'},hidden:false},
  ...
  for(const name of ['ragdoll-data.js','ragdoll-effects.js','ragdoll.js'])vm.runInNewContext(fs.readFileSync(path.join(root,name),'utf8'),sandbox);
  const pet=new (registered.get('ragdoll-game-pet'))();pet.isConnected=true;pet.connectedCallback();
