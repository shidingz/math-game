...
test('首次选择哪吒不影响悟空成长、共享积分及当前答题进度',()=>{
  ...
  assert.deepEqual(s.pets.nezha,{level:1,growth:0,feeds:0});assert.deepEqual(s.pets.wukong,{level:5,growth:86,feeds:14});assert.equal(s.points,170);assert.equal(s.round,round);assert.equal(s.grade,0);
});
test('喂养哪吒只改变哪吒，切换与存档恢复保留双方成长',()=>{
  ...
  const result=core.feed(s,s.activePet,registry().get('nezha'));assert.equal(result.status,'fed');assert.equal(s.points,180);assert.equal(s.pets.nezha.growth,20);assert.equal(s.pets.wukong.growth,0);
  core.selectPet(s,'wukong',['wukong','nezha']);core.feed(s);core.selectPet(s,'nezha',['wukong','nezha']);
  const saved=core.restore(JSON.parse(JSON.stringify(s)));assert.equal(saved.activePet,'nezha');assert.equal(saved.points,160);assert.deepEqual(saved.pets,s.pets);
});
test('旧悟空存档可添加哪吒且不重置原有进度',()=>{
  const s=core.restore({version:1,activePet:'wukong',points:80,pets:{wukong:{level:8,growth:30,feeds:40}}});
  const before=structuredClone(s.pets.wukong);assert.equal(core.selectPet(s,'nezha',['wukong','nezha']),false);s.points=380;assert.equal(core.unlockPet(s,'nezha',registry().get('nezha')).status,'unlocked');assert.ok(core.selectPet(s,'nezha',['wukong','nezha']));assert.deepEqual(s.pets.wukong,before);assert.equal(s.points,180);
});
test('200 积分兑换哪吒，积分不足或刷新都不能绕过兑换',()=>{
  ...
  locked.points=199;assert.equal(core.unlockPet(locked,'nezha',registry().get('nezha')).status,'insufficient');assert.equal(locked.points,199);
  locked.points=200;assert.equal(core.unlockPet(locked,'nezha',registry().get('nezha')).status,'unlocked');assert.equal(locked.points,0);assert.equal(core.selectPet(locked,'nezha',['wukong','nezha']),true);
...
test('未知或危险角色 ID 不修改存档，哪吒使用既定慢速成长曲线',()=>{
  ...
  const cfg=registry().get('nezha');assert.equal(core.totalAt(6,cfg),300);assert.equal(core.totalAt(11,cfg),1000);assert.equal(core.totalAt(15,cfg),1500);assert.equal(core.required(15,cfg),300);
});
