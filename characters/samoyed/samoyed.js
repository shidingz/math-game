/* Samoyed standalone Web Component. Load samoyed-data.js and samoyed-effects.js first. */
(() => {
  'use strict';
  const D=window.SamoyedGameData, FX=window.SamoyedEffects;
  if(!D||!FX)throw new Error('请先加载 samoyed-data.js 和 samoyed-effects.js');
  ...
  class SamoyedGamePet extends HTMLElement {
    ...
    constructor(){
      ...
      this.shadowRoot.innerHTML=`<style>
        ...
        canvas,.fx{position:absolute;inset:0;width:100%;height:100%}.fx{pointer-events:none;opacity:var(--fx-opacity,.34);filter:drop-shadow(0 0 var(--fx-glow,4px) #62b8ff);transition:opacity .3s,filter .3s}.fx svg{display:block;width:100%;height:100%}
        .fx.burst{opacity:1;filter:drop-shadow(0 0 10px #c2f4ff) drop-shadow(0 0 20px #397cec)}.shell.frozen .sy-motion,.shell.frozen .sy-motion *{animation-play-state:paused!important}
        canvas{touch-action:pan-y;outline:none}canvas:focus-visible{outline:3px solid #397bd2;outline-offset:-4px;border-radius:24px}
        ...
        @media(prefers-reduced-motion:reduce){.fx{transition:none}.sy-motion{animation:none!important}}
      </style><div class="shell"><div class="fx"></div><canvas width="960" height="680" role="button" tabindex="0" aria-label="萨摩耶，点击随机互动；左右方向键移动，空格互动"></canvas><span class="status" role="status">正在准备萨摩耶…</span></div>`;
    ...
    _emit(name,detail={}){this.dispatchEvent(new CustomEvent(name,{bubbles:true,composed:true,detail:{characterId:'samoyed',level:this.level,stage:this.stage,action:this.action,...detail}}));}
    async _load(afterLoad){
      const token=++this._token;this._loading=true;this.status.textContent='正在准备萨摩耶…';
      const base=new URL(this.getAttribute('asset-base')||'assets/',scriptBase);
      try{
        const im=await loadImage(new URL(`stage-${this.stage}.png`,base).href);
    ...
    play(name){
      ...
      this.canvas.setAttribute('aria-label',`萨摩耶，${D.stages[this.stage-1].name}，等级 ${this.level}，${D.actions[name].label}。点击随机互动。`);
      this._emit('pet-action');return this;
    }
    interact(){
      const event=new CustomEvent('pet-interact',{bubbles:true,composed:true,cancelable:true,detail:{characterId:'samoyed'}});
    ...
    _render(){
      ...
      if(['pet','feed'].includes(this.action)){
        c.fillStyle='#5f9df4';c.font='24px system-ui';for(let i=0;i<3;i++)c.fillText('♥',this._x+(i-1)*55,top+60-(reduced?0:(elapsed/40+i*15)%70));
      }
  ...
  if(!customElements.get('samoyed-game-pet'))customElements.define('samoyed-game-pet',SamoyedGamePet);
})();
