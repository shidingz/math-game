/* Siamese standalone Web Component. Load siamese-data.js and siamese-effects.js first. */
(() => {
  'use strict';
  const D=window.SiameseGameData, FX=window.SiameseEffects;
  if(!D||!FX)throw new Error('请先加载 siamese-data.js 和 siamese-effects.js');
  ...
  class SiameseGamePet extends HTMLElement {
    ...
    constructor(){
      ...
      this.shadowRoot.innerHTML=`<style>
        ...
        canvas,.fx{position:absolute;inset:0;width:100%;height:100%}.fx{pointer-events:none;opacity:var(--fx-opacity,.34);filter:drop-shadow(0 0 var(--fx-glow,4px) #56cfd0);transition:opacity .3s,filter .3s}.fx svg{display:block;width:100%;height:100%}
        .fx.burst{opacity:1;filter:drop-shadow(0 0 10px #f8dfa0) drop-shadow(0 0 20px #7a4bc6)}.shell.frozen .si-motion,.shell.frozen .si-motion *{animation-play-state:paused!important}
        canvas{touch-action:pan-y;outline:none}canvas:focus-visible{outline:3px solid #7a4bc6;outline-offset:-4px;border-radius:24px}
        ...
        @media(prefers-reduced-motion:reduce){.fx{transition:none}.si-motion{animation:none!important}}
      </style><div class="shell"><div class="fx"></div><canvas width="960" height="680" role="button" tabindex="0" aria-label="暹罗猫，点击随机互动；左右方向键移动，空格互动"></canvas><span class="status" role="status">正在准备暹罗猫…</span></div>`;
    ...
    _emit(name,detail={}){this.dispatchEvent(new CustomEvent(name,{bubbles:true,composed:true,detail:{characterId:'siamese',level:this.level,stage:this.stage,action:this.action,...detail}}));}
    async _load(afterLoad){
      const token=++this._token;this._loading=true;this.status.textContent='正在准备暹罗猫…';
    ...
    play(name){
      ...
      this.canvas.setAttribute('aria-label',`暹罗猫，${D.stages[this.stage-1].name}，等级 ${this.level}，${D.actions[name].label}。点击随机互动。`);
      this._emit('pet-action');return this;
    }
    interact(){
      const event=new CustomEvent('pet-interact',{bubbles:true,composed:true,cancelable:true,detail:{characterId:'siamese'}});
  ...
  if(!customElements.get('siamese-game-pet'))customElements.define('siamese-game-pet',SiameseGamePet);
})();
