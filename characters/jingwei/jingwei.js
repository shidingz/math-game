/* Jingwei standalone Web Component. Load jingwei-data.js and jingwei-effects.js first. */
(() => {
  'use strict';
  const D=window.JingweiGameData, FX=window.JingweiEffects;
  if(!D||!FX)throw new Error('请先加载 jingwei-data.js 和 jingwei-effects.js');
  ...
  class JingweiGamePet extends HTMLElement {
    ...
    constructor(){
      ...
      this.shadowRoot.innerHTML=`<style>
        ...
        .fx.burst{opacity:.9}.shell.frozen .jw-pulse{animation-play-state:paused!important}
        canvas{touch-action:pan-y;outline:none}canvas:focus-visible{outline:3px solid #198daa;outline-offset:-4px;border-radius:24px}
        ...
        @media(prefers-reduced-motion:reduce){.fx{transition:none}.jw-pulse{animation:none!important}}
      </style><div class="shell"><div class="fx"></div><canvas width="960" height="680" role="button" tabindex="0" aria-label="精卫，点击随机互动；左右方向键移动，空格互动"></canvas><span class="status" role="status">正在准备精卫…</span></div>`;
    ...
    _emit(name,detail={}){this.dispatchEvent(new CustomEvent(name,{bubbles:true,composed:true,detail:{characterId:'jingwei',level:this.level,stage:this.stage,action:this.action,...detail}}));}
    async _load(afterLoad){
      const token=++this._token;this._loading=true;this.status.textContent='正在准备精卫…';
    ...
    play(name){
      ...
      this.canvas.setAttribute('aria-label',`精卫，${D.stages[this.stage-1].name}，等级 ${this.level}，${D.actions[name].label}。点击随机互动。`);
      this._emit('pet-action');return this;
    }
    interact(){
      const event=new CustomEvent('pet-interact',{bubbles:true,composed:true,cancelable:true,detail:{characterId:'jingwei'}});
    ...
    _render(){
      ...
      if(['pet','feed'].includes(this.action)){
        c.fillStyle='#ec8690';c.font='24px system-ui';for(let i=0;i<3;i++)c.fillText('♥',this._x+(i-1)*55,top+60-(reduced?0:(elapsed/40+i*15)%70));
      }
  ...
  if(!customElements.get('jingwei-game-pet'))customElements.define('jingwei-game-pet',JingweiGamePet);
})();
