/* Ragdoll standalone Web Component. Load ragdoll-data.js and ragdoll-effects.js first. */
(() => {
  'use strict';
  const D=window.RagdollGameData, FX=window.RagdollEffects;
  if(!D||!FX)throw new Error('请先加载 ragdoll-data.js 和 ragdoll-effects.js');
  ...
  class RagdollGamePet extends HTMLElement {
    ...
    constructor(){
      ...
      this.shadowRoot.innerHTML=`<style>
        ...
        .fx.burst{opacity:1;filter:drop-shadow(0 0 10px #e9f6ff) drop-shadow(0 0 20px #438bdc)}.shell.frozen .rg-motion,.shell.frozen .rg-motion *{animation-play-state:paused!important}
        ...
        @media(prefers-reduced-motion:reduce){.fx{transition:none}.rg-motion{animation:none!important}}
      </style><div class="shell"><div class="fx"></div><canvas width="960" height="680" role="button" tabindex="0" aria-label="布偶猫，点击随机互动；左右方向键移动，空格互动"></canvas><span class="status" role="status">正在准备布偶猫…</span></div>`;
    ...
    _emit(name,detail={}){this.dispatchEvent(new CustomEvent(name,{bubbles:true,composed:true,detail:{characterId:'ragdoll',level:this.level,stage:this.stage,action:this.action,...detail}}));}
    async _load(afterLoad){
      const token=++this._token;this._loading=true;this.status.textContent='正在准备布偶猫…';
      const base=new URL(this.getAttribute('asset-base')||'assets/',scriptBase);
      try{
        const im=await loadImage(new URL(`stage-${this.stage}.png?v=frost2`,base).href);
    ...
    play(name){
      ...
      this.canvas.setAttribute('aria-label',`布偶猫，${D.stages[this.stage-1].name}，等级 ${this.level}，${D.actions[name].label}。点击随机互动。`);
      this._emit('pet-action');return this;
    }
    interact(){
      const event=new CustomEvent('pet-interact',{bubbles:true,composed:true,cancelable:true,detail:{characterId:'ragdoll'}});
  ...
  if(!customElements.get('ragdoll-game-pet'))customElements.define('ragdoll-game-pet',RagdollGamePet);
})();
