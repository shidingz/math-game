/* Yutu standalone Web Component. Load yutu-data.js and yutu-effects.js first. */
(() => {
  'use strict';
  const D=window.YutuGameData, FX=window.YutuEffects;
  if(!D||!FX)throw new Error('请先加载 yutu-data.js 和 yutu-effects.js');
  ...
  class YutuGamePet extends HTMLElement {
    ...
    _emit(name,detail={}){this.dispatchEvent(new CustomEvent(name,{bubbles:true,composed:true,detail:{characterId:'yutu',level:this.level,stage:this.stage,action:this.action,...detail}}));}
    async _load(afterLoad){
      const token=++this._token;this._loading=true;this.status.textContent='正在准备玉兔…';
    ...
    play(name){
      ...
      this.canvas.setAttribute('aria-label',`玉兔，${D.stages[this.stage-1].name}，等级 ${this.level}，${D.actions[name].label}。点击随机互动。`);
      this._emit('pet-action');return this;
    }
    interact(){
      const event=new CustomEvent('pet-interact',{bubbles:true,composed:true,cancelable:true,detail:{characterId:'yutu'}});
    ...
    _render(){
      ...
      if(['pet','feed'].includes(this.action)){
        c.fillStyle='#9c8dea';c.font='24px system-ui';for(let i=0;i<3;i++)c.fillText('♥',this._x+(i-1)*55,top+60-(reduced?0:(elapsed/40+i*15)%70));
      }
  ...
  if(!customElements.get('yutu-game-pet'))customElements.define('yutu-game-pet',YutuGamePet);
})();
