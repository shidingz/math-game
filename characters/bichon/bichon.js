/* Bichon standalone Web Component. Load bichon-data.js and bichon-effects.js first. */
(() => {
  'use strict';
  const D=window.BichonGameData, FX=window.BichonEffects;
  if(!D||!FX)throw new Error('请先加载 bichon-data.js 和 bichon-effects.js');
  ...
  class BichonGamePet extends HTMLElement {
    ...
    constructor(){
      ...
      this.shadowRoot.innerHTML=`<style>
        ...
        .fx.burst{opacity:1;filter:drop-shadow(0 0 10px #e9f6ff) drop-shadow(0 0 20px #438bdc)}.shell.frozen .bi-motion,.shell.frozen .bi-motion *{animation-play-state:paused!important}
        ...
        @media(prefers-reduced-motion:reduce){.fx{transition:none}.bi-motion{animation:none!important}}
      </style><div class="shell"><div class="fx"></div><canvas width="960" height="680" role="button" tabindex="0" aria-label="比熊犬，点击随机互动；左右方向键移动，空格互动"></canvas><span class="status" role="status">正在准备比熊犬…</span></div>`;
    ...
    _emit(name,detail={}){this.dispatchEvent(new CustomEvent(name,{bubbles:true,composed:true,detail:{characterId:'bichon',level:this.level,stage:this.stage,action:this.action,...detail}}));}
    async _load(afterLoad){
      const token=++this._token;this._loading=true;this.status.textContent='正在准备比熊犬…';
    ...
    play(name){
      ...
      this.canvas.setAttribute('aria-label',`比熊犬，${D.stages[this.stage-1].name}，等级 ${this.level}，${D.actions[name].label}。点击随机互动。`);
      this._emit('pet-action');return this;
    }
    interact(){
      const event=new CustomEvent('pet-interact',{bubbles:true,composed:true,cancelable:true,detail:{characterId:'bichon'}});
  ...
  if(!customElements.get('bichon-game-pet'))customElements.define('bichon-game-pet',BichonGamePet);
})();
