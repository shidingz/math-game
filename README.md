...
## 当前版本：经典布局＋照片制作（2026-09-18）

当前开发版本为 `wechat-v23-classic-home`。浏览器首页恢复经典浅色伙伴卡片，手机按正常尺寸滚动、桌面双栏，伙伴放大取景。拍照制作、改名、独立成长和伙伴打招呼继续保留；微信原生布局不随此次网页改版变化。以下照片服务为 v22 引入且继续复用。新增真正使用相机/相册、IndexedDB 和 HTTP 的 Canvas 网页客户端 `wechat/browser/`，以及在本机电脑运行的 `server/` 制作服务。灰猫案例已通过 4 次第三方生图请求生成三阶段、27 个动作；当前构建为 8 个内置伙伴加 1 个灰猫「新伙伴」。原根目录网页仍保留，两个网页入口不要混用。
